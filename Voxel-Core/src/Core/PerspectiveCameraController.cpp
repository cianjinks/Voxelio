...
{
	if (VoxelCore::Input::IsKeyPressed(GLFW_KEY_W)) {
		m_CameraPos += m_CameraSpeed * m_CameraFront;
	}
	if (VoxelCore::Input::IsKeyPressed(GLFW_KEY_S)) {
		m_CameraPos -= m_CameraSpeed * m_CameraFront;
	}
	if (VoxelCore::Input::IsKeyPressed(GLFW_KEY_A)) {
		m_CameraPos -= glm::normalize(glm::cross(m_CameraFront, m_CameraUp)) * m_CameraSpeed;
	}
	if (VoxelCore::Input::IsKeyPressed(GLFW_KEY_D)) {
		m_CameraPos += glm::normalize(glm::cross(m_CameraFront, m_CameraUp)) * m_CameraSpeed;
	}

	if (VoxelCore::Input::IsKeyPressed(GLFW_KEY_LEFT_SHIFT))
	{
		m_CameraSpeed = 5.0f * VoxelCore::Timestep::GetDeltaTime();
	}
	else
	{
		m_CameraSpeed = 2.5f * VoxelCore::Timestep::GetDeltaTime();
	}
	CalculateMatrices();
}
